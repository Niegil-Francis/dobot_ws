...
def coordinates(match):
	...
	dn=-53
	d={}
	d["A"]=np.array([[0,0,up],[0,0,dn],[-1.5,-3,dn],[-3,0,dn]])

	d["B"]=np.array([[0,-3,up],[0,-3,dn],[-3,-3,dn],[-3,-1.5,dn],[0,-1.5,dn],[0,-1.5,up],[3,-1.5,up],[-3,-1.5,dn],[-3,0,dn],[0,0,dn],[0,0,up],[-1,-3,up],[-1,-3,dn],[-1,0,dn]])
	...
	d["P"]=np.array([[-1,-1.5,up],[-1,-1.5,dn],[-3,-1.5,dn],[-3,-3,dn],[-1,-3,dn],[-1,0,dn]])
		...
		try: 
			...
			if(sel=='y'):
				...
				for string in strings:
					word="".join(ch.upper() for ch in string if ch.isalpha())
					res=[]
					res[:] = word
					num=len(string)
					num=num//2
